...
@media print {
  ...
  // Build the logo <img> HTML for print windows (needs absolute URL)
  const getLogoHtml = (size = 200) => {
    const fullUrl = window.location.origin + LOGO_PATH;
    return `<img src="${fullUrl}" alt="Company Logo" style="display:block; margin:0 auto 10px auto; width:${size}px; height:${size}px; object-fit:contain;" />`;
  };
...
<div style="display:flex; justify-content:space-between; font-size:13px; margin:20px 0; padding-bottom:18px; border-bottom:1px solid #000; position:relative; z-index:1;">
  <div>
...
<tr>
  ...
  <td style="text-align:center; padding:8px 6px; border-bottom:1px solid #ddd;">${item.quantity}</td>
    ...
    const waybillTable = `
<table style="width:100%; border-collapse:collapse; margin:20px 0; font-size:13px; position:relative; z-index:1;">
  <thead>
...
<div style="margin-top:60px; position:relative; z-index:1;">
  ...
  <div class="waybill-thanks" style="text-align:center; margin-top:50px; font-size:14px; font-weight:600;">
    Goods received in good condition. No liability after delivery.
  </div>

  <div class="waybill-thanks" style="text-align:center; margin-top:30px; font-size:14px; font-weight:600;">
    Thank you for your business
  </div>
</div>
    `;

    const watermarkHtml = `<img src="${window.location.origin + LOGO_PATH}" alt="" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%) rotate(-30deg); width:400px; height:auto; opacity:0.12; pointer-events:none; z-index:0;" />`;
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-10">
        ...
        {sale && (
          <div className="bg-slate-900 text-slate-100 rounded-xl shadow-2xl overflow-hidden ring-1 ring-slate-700/70 border border-slate-700">
            <div className="p-10 md:p-12" ref={receiptRef} style={{ position: "relative", overflow: "hidden" }}>
              {/* Watermark */}
              <img
                src={window.location.origin + LOGO_PATH}
                alt=""
                className="receipt-watermark"
                style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%) rotate(-30deg)", width: 400, height: "auto", opacity: 0.12, pointerEvents: "none", zIndex: 0 }}
              />
              ...
              <div className="text-center mb-10" style={{ position: "relative", zIndex: 1 }}>
                <img
                  ...
                  style={{ width: 200, height: 200, objectFit: "contain", display: "block", margin: "0 auto 10px auto" }}
                />
              ...
              {/* Thank You */}
              <div className="thanks" style={{ position: "relative", zIndex: 1 }}>
                Thank you for your patronage — Come again!
              </div>

              {/* Legal */}
              <div className="legal" style={{ position: "relative", zIndex: 1 }}>
