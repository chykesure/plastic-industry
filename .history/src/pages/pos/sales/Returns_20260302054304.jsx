...
function Returns() {
  ...
  const BASE_URL_ENDPOINT = "http://localhost:8080/api";
  ...
  const fetchSale = async () => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL_ENDPOINT}/returns/sale/invoice/${invoiceNumber.trim()}`
      );
  ...
  const submitReturn = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${BASE_URL_ENDPOINT}/returns`, {
        saleId: sale._id,
        items: itemsToReturn,
        refundMethod,
        reason: reason.trim(),
