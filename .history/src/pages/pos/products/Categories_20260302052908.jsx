...
function Categories() {
  ...
  const API_BASE_ENDPOINT = "http://localhost:8080/api/categories";
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      setLoading(true);
      try {
        const res = await fetch(API_BASE_ENDPOINT);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_ENDPOINT}/${editId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_ENDPOINT}/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (res.ok) {
