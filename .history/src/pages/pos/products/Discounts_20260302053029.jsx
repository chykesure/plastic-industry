...
function ExpiryManagement() {
  ...
  const API_BASE_ = "http://localhost:8080/api";
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`${API_BASE_}/products`);
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_}/expiry`);
      setRecords(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editItem) {
        await axios.put(`${API_BASE_}/expiry/${editItem._id}`, formData);
        toast.success("Expiry record updated!");
      } else {
        await axios.post(`${API_BASE_}/expiry`, formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_}/expiry/${id}`);
