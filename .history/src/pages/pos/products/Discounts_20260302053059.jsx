...
function ExpiryManagement() {
  ...
  const API_BASE_ENDPOINT = `${API_BASE}/api`;
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`${API_BASE_ENDPOINT}/products`);
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_ENDPOINT}/expiry`);
      setRecords(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editItem) {
        await axios.put(`${API_BASE_ENDPOINT}/expiry/${editItem._id}`, formData);
        toast.success("Expiry record updated!");
      } else {
        await axios.post(`${API_BASE_ENDPOINT}/expiry`, formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_ENDPOINT}/expiry/${id}`);
