...
function Categories() {
  ...
  const API_BASE_ = "http://localhost:8080/api/categories";
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      setLoading(true);
      try {
        const res = await fetch(API_BASE_);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_}/${editId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_}/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (res.ok) {
